namespace :kura do
  desc "List users (id, email, events, birthdays, created)"
  task users: :environment do
    ...
    puts format("%-4s  %-36s  %6s  %9s  %s", "id", "email", "events", "birthdays", "created")
    User.order(:id).each do |user|
      puts format("%-4s  %-36s  %6s  %9s  %s",
        user.id,
        user.email,
        user.events.count,
        user.birthdays.count,
